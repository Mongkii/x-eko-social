...
// AI flow imports will be removed/changed if DeepSeek is used directly without Genkit
...
const LOCAL_STORAGE_LIKED_ADS_KEY = 'eko_liked_drop_ids'; // Updated key name
...
export default function HomePage() {
  const [feedItems, setFeedItems] = useState<FeedItemData[]>([]); // Use FeedItemData for UI compatibility for now
  const [isLoading, setIsLoading] = useState(true);
  const [followedCategories, setFollowedCategories] = useState<Set<string>>(new Set(['Travel'])); // Example
  ...
  useEffect(() => {
    ...
    if (androidBridge && typeof androidBridge.getSubscriptionStatus === 'function') {
      ...
      setAvailableIAPs(mockInAppPurchaseItems); // Simulating fetching IAPs
  ...
  useEffect(() => {
    ...
    // Map EkoDrop data to FeedItemData for UI compatibility
    const mappedInitialFeed = initialFeedItems.map((item: EkoDrop): FeedItemData => ({
      id: item.id,
      type: item.isAd ? 'ad' : 'content',
      audioUrl: item.audioURL, // EkoDrop uses audioURL
      videoUrl: item.audioURL, // using audioURL for video element src
      posterUrl: item.rawAudioURL, // Example: using rawAudioURL as poster, adjust as needed
      dataAiHint: 'voice audio', // Generic hint
      title: item.transcript?.['en']?.substring(0,50) || `EkoDrop ${item.id}`, // Example title from transcript
      description: item.transcript?.['en'] || `Audio by user ${item.userId}`,
      categories: item.hashtags || [],
      userRating: 0, // Default or fetch if available
      ...
      createdAt: item.createdAt, // Keep as Firestore Timestamp or convert
  ...
  const handleAudioEnded = (endedItemId: string) => {
    ...
    if (currentIndex !== -1 && currentIndex < feedItems.length - 1) {
      ...
      setCurrentPlayingId(null); // End of feed
    }
  };
    ...
    setFeedItems(prevItems =>
      prevItems.map(item => {
        if (item.id === itemId) {
          ...
          itemTitle = item.title;
          const newIsLiked = item.isLiked ? false : item.isLiked; // Unlike if disliked
  ...
  const handlePersonalizeFeed = async () => {
    ...
    try {
      const result = await personalizeAdFeedDeepSeek({ // Using simulated DeepSeek function
        userPastInteractions: interactionsString || "No specific interactions yet.",
        availableAds: availableAdsString,
      });
      
      const newFeed = [...feedItems]; // Start with current items
      // Simple shuffle for demo purposes
      for (let i = newFeed.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [newFeed[i], newFeed[j]] = [newFeed[j], newFeed[i]];
      }
      // Potentially add an AI mentioned ad if not present
      const aiMentionedAdTitle = result.personalizedAdFeed.split(',')[0].trim(); // Example parsing
      ...
      setFeedItems(newFeed.slice(0, initialFeedItems.length)); // Keep feed length consistent
      toast({
    ...
    try {
      ...
      if (isAndroidApp && window.androidAppBridge?.subscribe) {
        ...
        result = await window.iosAppBridge.subscribe(sku);
         ...
         // Simulate success for web testing
  ...
  const handlePurchaseProduct = async (sku: string, title: string) => {
     toast({ title: t('purchasingItem', {item: title}), description: t('pleaseWait') }); // Ensure 'item' matches translation key
     try {
      let result = { success: false, message: "Bridge not found" };
      ...
      if (result.success) {
        toast({ title: t('purchaseSuccessful'), description: t('youHavePurchasedItem', {item: title}) }); // Ensure 'item' matches
      } else {
        toast({ variant: "destructive", title: t('purchaseFailed'), description: result.message || t('couldNotPurchaseItem', {item: title}) }); // Ensure 'item' matches
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main className="flex-grow overflow-y-auto snap-y snap-mandatory">
        {(isAndroidApp || isIOSApp) && ( // Show if either bridge is detected
        ...
        {isLoading ? (
          ...
          feedItems.map((item, index) => (
            <div key={`${item.id}-${index}`} className="h-full"> {/* Ensure each feed item div takes full height */}
              <FeedItemCard
                item={item}
